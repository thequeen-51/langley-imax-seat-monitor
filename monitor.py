...
STATE_FILE = Path(".monitor-state/active_matches.json")
...
def clean_api_headers(
    ...
    """Keep headers needed for Cineplex API requests."""
...
def load_previous_match_keys() -> set[str]:
    """Load matches that were active during the previous run."""
...
def save_current_match_keys(
    match_keys: set[str],
) -> None:
    """Persist currently active matches for deduplication."""
...
def build_seat_lookup(
    ...
    to a visible label such as H12.
    """
...
def find_preferred_pair(
    ...
    """
    Find the highest-priority adjacent available pair.
    """
    ...
    preferred_available = sorted(
        ...
        if (
            re.fullmatch(r"[A-Z]+\d+", label)
            and re.match(r"[A-Z]+", label)
            and re.match(
                r"[A-Z]+",
                label,
            ).group(0) in monitored_rows
        )
    )

    print(
        "Available monitored-area seats:",
        preferred_available,
    )
...
def make_match_key(
    ...
    """Create a stable identifier for one seat match."""
...
def main() -> None:
    """Run the Langley IMAX 70mm seat monitor."""
    ...
    previous_match_keys = (
        load_previous_match_keys()
    )

    print(
        "Previous active matches:",
        len(previous_match_keys),
    )
    ...
    with sync_playwright() as playwright:
        ...
        current_match_keys: set[str] = set()
        ...
        for index, session in enumerate(
            ...
            current_match_keys.add(match_key)

            if match_key in previous_match_keys:
                print(
                    "Already notified while this "
